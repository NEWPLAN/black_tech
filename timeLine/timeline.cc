...
TimelineWriter::TimelineWriter()
{
  ...
  record_queue_ = new SPSCQueue<TimelineRecord>(1024);
}
...
{
  {
    ...
    {
      LOG(INFO) << "Current filename for timeline is same as new filename. , Returning.";
      return;
    }
  ...
  while (true)
  {
    ...
    if (filename == "")
    {
      LOG(INFO) << "StopTimeline is called. Blocking thread since "
                   "pending_status is still true.\n";
    }
    else
    {
      LOG(INFO) << "StartTimeline is called. Blocking thread since "
                   "pending_status is still true.\n";
    }
...
void TimelineWriter::SetTimelineFile(std::string filename)
{
  ...
  if (cur_filename_ != "" && cur_filename_ != filename)
  {
    ...
    {
      LOG(INFO) << " SetTimelineFile is no-op as there are events in "
  ...
  {
    LOG(INFO) << "Error opening the Horovod Timeline file " << filename
...
void TimelineWriter::WriterLoop()
{
  while (healthy())
  {
    ...
    }
    {
      ...
      {
        LOG(INFO) << "Error writing to the Horovod Timeline after it was "
                              ...
                              const RequestType request_type)
{
  ...
  std::string event_category = "NEGOTIATE_";
  if (request_type == READ_REMOTE)
    event_category += "READ_REMOTE";
                     ...
                     const ResponseType response_type)
{
  ...
  std::lock_guard<std::recursive_mutex> guard(mutex_);
  assert(tensor_states_[tensor_name] == TimelineState::UNKNOWN);
  auto event_category = "ResponseType_Name(response_type)";
  WriteEvent(tensor_name, 'B', event_category);
  tensor_states_[tensor_name] = TimelineState::TOP_LEVEL;
}

void Timeline::ActivityStartAll(const std::vector<My_TensorTableEntry> &entries,
                                const std::string &activity)
{
  if (!Initialized() || !writer_.active())
  {
    LOG(INFO) << "Not activated" << std::endl;
    return;
  }
  for (auto &e : entries)
  {
                             ...
                             const std::string &activity)
{
  if (!Initialized() || !writer_.active())
  {
    LOG(INFO) << "Not ActivityStart";
    return;
  }
  LOG(INFO) << "ActivityStart: " << tensor_name << ", level: " << tensor_states_[tensor_name];
  std::lock_guard<std::recursive_mutex> guard(mutex_);
...
void Timeline::ActivityEndAll(const std::vector<My_TensorTableEntry> &entries)
{
  for (auto &e : entries)
  {
...
void Timeline::End(const std::string &tensor_name, std::string *tensor)
{
  if (!Initialized() || !writer_.active())
  {
    return;
